...
def update_stok():
    try:
        data = request.json
        inventory = Inventory.query.filter_by(sku=data['sku'], batch_number=data['batch_number']).first()
...
def add_inventory():
    try:
        ...
        db.session.commit()
        return jsonify({'message': 'Item berhasil ditambahkan', 'sku': inventory.sku, 'batch_number': inventory.batch_number}), 201
    except Exception as e:
